...
from tools.M_evidence import MutagenicityEvidenceTool
...
class ComprehensiveMutagenicityTool(BaseTool):
    name: str = "ComprehensiveMutagenicityTool"
    description: str = (
        "Comprehensive mutagenicity analysis tool that takes a chemical name, CAS number, or SMILES notation "
        "and provides: 1) Computational prediction of mutagenicity, 2) Literature evidence from databases, "
    ...
    evidence_tool: MutagenicityEvidenceTool
    ...
    def __init__(self, llm: LLM, **kwargs):
        cmr_tool = CMRPrediction()
        evidence_tool = MutagenicityEvidenceTool(llm=llm)
    ...
    def _predict_mutagenicity(self, smiles: str) -> Dict[str, Any]:
        try:
            print(f"Step 2: Predicting mutagenicity for SMILES: {smiles}")

            query = f"{{'smiles': ['{smiles}'], 'property_type': 'M'}}"
            ...
            return {
                'success': False,
                'error': f"Error in mutagenicity prediction: {str(e)}",
                'smiles': smiles
            }

    def _get_evidence(self, identifier: str) -> Dict[str, Any]:
        try:
            print(f"Step 3: Retrieving mutagenicity evidence for: {identifier}")
    ...
    def _generate_comprehensive_summary(self,
        ...
        try:
            ...
            prompt_template = PromptTemplate(
                template="""
                Please provide a comprehensive mutagenicity assessment for the following compound based on both computational predictions and literature evidence.
                ...
                3. Literature Evidence: Summary of experimental evidence (Ames test, in vitro/in vivo studies)
                4. Integrated Conclusion: Overall mutagenicity assessment considering both sources
            ...
            if summary_data['computational_prediction'].get('prediction_available'):
                ...
                label_text = "Mutagenic" if pred['label'] == 1 else "Non-mutagenic"
    ...
    def _run(self, identifier: str) -> str:
        try:
            print(f"Starting comprehensive mutagenicity analysis for: {identifier}")
            print("=" * 60)
            ...
            prediction_result = self._predict_mutagenicity(smiles)
            ...
            error_msg = f"Error in comprehensive mutagenicity analysis: {str(e)}"
            print(error_msg)
            return error_msg
