...
from tools.R_evidence import ReproductiveToxicityEvidenceTool
...
class ComprehensiveReproductiveToxicityTool(BaseTool):
    name: str = "ComprehensiveReproductiveToxicityTool"
    description: str = (
        "Comprehensive reproductive toxicity analysis tool that takes a chemical name, CAS number, or SMILES notation "
        "and provides: 1) Computational prediction of reproductive toxicity, 2) Literature evidence from databases, "
    ...
    evidence_tool: ReproductiveToxicityEvidenceTool
    ...
    def __init__(self, llm: LLM, **kwargs):
        cmr_tool = CMRPrediction()
        evidence_tool = ReproductiveToxicityEvidenceTool(llm=llm)
    ...
    def _predict_reproductive_toxicity(self, smiles: str) -> Dict[str, Any]:
        try:
            print(f"Step 2: Predicting reproductive toxicity for SMILES: {smiles}")
            query = f"{{'smiles': ['{smiles}'], 'property_type': 'R'}}"
            ...
            return {
                'success': False,
                'error': f"Error in reproductive toxicity prediction: {str(e)}",
                'smiles': smiles
            }

    def _get_evidence(self, identifier: str) -> Dict[str, Any]:
        try:
            print(f"Step 3: Retrieving reproductive toxicity evidence for: {identifier}")
    ...
    def _generate_comprehensive_summary(self,
        ...
        try:
            ...
            prompt_template = PromptTemplate(
                template="""
                Please provide a comprehensive reproductive toxicity assessment for the following compound based on both computational predictions and literature evidence.
                ...
                2. Computational Assessment: Interpretation of the reproductive toxicity prediction results
                3. Literature Evidence: Summary of experimental/epidemiological evidence related to reproductive health
                4. Integrated Conclusion: Overall reproductive toxicity assessment considering both sources, including:
            ...
            if summary_data['computational_prediction'].get('prediction_available'):
                ...
                label_text = "Reproductive Toxic" if pred['label'] == 1 else "Non-reproductive Toxic"
    ...
    def _run(self, identifier: str) -> str:
        try:
            print(f"Starting comprehensive reproductive toxicity analysis for: {identifier}")
            print("=" * 60)
            ...
            prediction_result = self._predict_reproductive_toxicity(smiles)
            ...
            error_msg = f"Error in comprehensive reproductive toxicity analysis: {str(e)}"
            print(error_msg)
            return error_msg
