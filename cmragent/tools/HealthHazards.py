...
class HealthHazards(BaseTool):
    ...
    def __init__(self, llm):
        ...
        prompt = PromptTemplate(
            template="Extract and summarize health hazards and safety information from the data: {data}",
    ...
    def _extract_health_and_safety_info(self, section):
        if isinstance(section, list):
            for sec in section:
                result = self._extract_health_and_safety_info(sec)
                if result:
                    return result
        elif isinstance(section, dict):
            heading = section.get("TOCHeading")
            if heading in ["Explosion Hazards", "Fire Hazards", "Fire Potential", "Hazards Summary", "Health Hazards", "Skin, Eye, and Respiratory Irritations"]:
            ...
            if "Section" in section:
                return self._extract_health_and_safety_info(section["Section"])
        return None

    def _run(self, input_str: str) -> str:
        print(f"Running HealthHazardsTool with input: {input_str}")
        ...
        try:
            ...
            health_and_safety_section = data.get("Record", {}).get("Section", [])
            if not health_and_safety_section:
                return "No health and safety information found."

            result = self._extract_health_and_safety_info(health_and_safety_section)
            return result if result else "No health and safety information found."

        except KeyError as e:
            print(f"Error parsing the health and safety data: {e}")
            return "Error parsing the safety data."
